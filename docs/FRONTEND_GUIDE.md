...
- 地块画像页采用三栏布局：左侧基础信息（260px）、中间指标快照网格（auto-fill）、右侧单指标趋势图（320px+）。底部左右两栏分别是观测批次表和预警建议面板。
- 地块画像指标快照卡包含指标名称、当前值+单位、StatusTag、较上次观测变化（箭头+百分比），左边条颜色按状态等级区分。
- 趋势图支持指标选择下拉和近7/15/30天/全部时间范围切换，支持 markArea 显示指标正常范围。
- 预警与建议区域展示当前地块相关预警并附简洁建议措施，不编造过度具体的农艺处方。
- 从地图页点击"查看画像"进入地块画像，"定位到地图"按钮回到 `/map-twin?plotId=xxx`。
