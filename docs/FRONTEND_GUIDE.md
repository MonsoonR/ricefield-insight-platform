...
- 场景驾驶舱使用地图核心工作台承载场景总览，避免回退到大横幅和 KPI 卡片堆叠的普通后台首页。
- 场景驾驶舱首页应优先复用现有 `overview`、`metrics`、`metric-compare`、`plot-series` 和 `warnings` API，不为了视觉效果新增复杂后端逻辑。
- 场景驾驶舱核心结构固定为：左侧地块 / 图层 rail、中央 Cesium 地图、右侧地块 inspector、底部分析 dock；窄屏下优先展示地图，再展示 inspector 与 rail。
...
- 移动端全局 App Shell 保留顶部栏，显示当前页面标题、预警入口、系统说明入口和演示用户标识；侧边栏只在桌面端显示。
